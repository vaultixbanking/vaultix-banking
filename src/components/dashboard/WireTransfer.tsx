  ...
  if (success) {
    return (
      <div className="max-w-lg mx-auto text-center py-16">
        ...
        <p className="text-secondary-500 mb-2">Your wire transfer of <span className="font-semibold">${parseFloat(form.amount).toLocaleString()}</span> is being processed.</p>
        <p className="text-xs text-secondary-400 mb-6">International wire transfers may take 3-5 business days.</p>
        <button onClick={() => { setForm({ amount: '', beneficiaryName: '', bankName: '', accountNumber: '', country: '', swiftCode: '', routingNumber: '', accountType: 'savings', narration: '' }); setSuccess(false); }} className="px-6 py-2.5 bg-primary-600 text-white rounded-xl font-medium hover:bg-primary-700 transition-colors">
          New Transfer
        </button>
      </div>
    );
      ...
      <div className="bg-white rounded-2xl border border-secondary-100 shadow-sm overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div className="p-3 bg-blue-50 border border-blue-200 rounded-xl flex items-start gap-2">
            ...
            <p className="text-sm text-blue-700">Wire transfers are subject to review and may take 3-5 business days to settle.</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={form.accountNumber} onChange={e => update('accountNumber', e.target.value)} placeholder="Account number or IBAN" className="w-full px-4 py-3 border border-secondary-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent" />
            </div>
            <div>
              <label className="block text-sm font-medium text-secondary-700 mb-2">Country</label>
              <input type="text" value={form.country} onChange={e => update('country', e.target.value)} placeholder="Country" className="w-full px-4 py-3 border border-secondary-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-secondary-700 mb-2">SWIFT / BIC Code</label>
              <input type="text" value={form.swiftCode} onChange={e => update('swiftCode', e.target.value)} placeholder="SWIFT code" className="w-full px-4 py-3 border border-secondary-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent" />
            </div>
            <div>
              <label className="block text-sm font-medium text-secondary-700 mb-2">Routing Number</label>
              <input type="text" value={form.routingNumber} onChange={e => update('routingNumber', e.target.value)} placeholder="Routing number (optional)" className="w-full px-4 py-3 border border-secondary-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent" />
            </div>
          </div>
          ...
          <button type="submit" disabled={loading} className="w-full py-3 bg-primary-600 text-white rounded-xl font-semibold hover:bg-primary-700 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
            {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Processing...</> : 'Send Wire Transfer'}
          </button>
        </form>
      </div>
    </div>
